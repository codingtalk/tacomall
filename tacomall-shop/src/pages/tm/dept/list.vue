 ...
 * @LastEditTime: 2021-10-14 16:57:21
 * @LastEditors: 码上talk|RC
 * @Description:
 * @FilePath: /tacomall-web/src/pages/tm/dept/list.vue
 * @微信:  13680065830
        ...
        tmDeptUpdate: {
          ...
          fn: {
            ...
            openHandler: (ok, params) => {
              ...
              TmDept.sendApi('deptInfo', { params: {}, data: { query: { id: params.row._raw.id } } }).then(res => {
                const { status, data } = res;
                if (status) {
            ...
            closeHandler: (okCallback, errorCallback) => {
              ...
              if (ok) {
                TmDept.sendApi('deptUpdate', { params: { id: this.popup.tmDeptUpdate.attr.info.id }, data: form }).then(
                  (res) => {
                    const { status, data } = res;
                    if (status) {
                      okCallback();
