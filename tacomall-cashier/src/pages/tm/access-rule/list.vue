 ...
 * @LastEditTime: 2021-10-14 16:55:20
 * @LastEditors: 码上talk|RC
 * @Description:
 * @FilePath: /tacomall-web/src/pages/tm/access-rule/list.vue
 * @微信:  13680065830
        ...
        tmAccessRuleUpdate: {
          ...
          fn: {
            ...
            openHandler: (ok, params) => {
              TmAccessRule.sendApi('accessRuleInfo', { params: {}, data: { query: { id: params.row._raw.id } } }).then(res => {
                const { status, data } = res;
                if (status) {
            ...
            closeHandler: (okCallback, errorCallback) => {
              ...
              if (ok) {
                TmAccessRule.sendApi('accessRuleUpdate', { params: { id: this.popup.tmAccessRuleUpdate.attr.info.id }, data: form }).then(
                  (res) => {
                    const { status, data } = res;
                    if (status) {
                      okCallback();
