 ...
 * @LastEditTime: 2021-10-14 16:58:47
 * @LastEditors: 码上talk|RC
 * @Description:
 * @FilePath: /tacomall-web/src/pages/tm/staff/list.vue
 * @微信:  13680065830
        ...
        tmStaffUpdate: {
          ...
          fn: {
            ...
            openHandler: (ok, params) => {
              TmStaff.sendApi('staffInfo', { params: {}, data: { query: { id: params.row._raw.id } } }).then(res => {
                const { status, data } = res;
                if (status) {
            ...
            closeHandler: (okCallback, errorCallback) => {
              ...
              if (ok) {
                TmStaff.sendApi('staffUpdate', { params: { id: this.popup.tmStaffUpdate.attr.info.id }, data: form }).then(
                  (res) => {
                    const { status, data } = res;
                    if (status) {
                      okCallback();
